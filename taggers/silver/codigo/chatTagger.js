  ...
  iterateTagChats(chatDivs) {
    ...
    async function procesarChat() {
      ...
      setTimeout(async () => {
        ...
        // PASO 3: Scrollear al inicio de la conversación
        const chatWindow = document.querySelector('.mui-npbckn');
        if (chatWindow) {
          chatWindow.scrollTop = 0;
          console.log(`2️⃣ STEP 2: Scrolleado al INICIO de la conversación`);
        }
        
        // Esperar a que se estabilice y cargue todo el contenido
        ...
        if (!seGuardó) {
          ...
          // 🔄 Limpiar duplicados basándose en DD-MM-ID (ignorando letra y signo)
          const limpiarDuplicados = (lista) => {
            const mapa = {};
            
            lista.forEach(codigo => {
              const base = codigo.replace(/[A-Z!]*$/, ''); // Extrae DD-MM-ID sin letra ni !
              
              if (!mapa[base]) {
                mapa[base] = codigo;
              } else {
                // Si ya existe, mantener la versión con ! si la nueva la tiene
                const existente = mapa[base];
                const nuevoTieneSigno = codigo.endsWith('!');
                const existenteTieneSigno = existente.endsWith('!');
                
                if (nuevoTieneSigno && !existenteTieneSigno) {
                  console.log(`   🔄 DUPLICATE CLEANUP: "${existente}" → "${codigo}"`);
                  mapa[base] = codigo;
                }
              }
            });
          ...
          const codigosSinDuplicados = limpiarDuplicados(codigos);
          console.log(`   Códigos sin duplicados: [${codigosSinDuplicados.join(', ')}]`);
          
          const nuevoValor = codigosSinDuplicados.join(', ');
